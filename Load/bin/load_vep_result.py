...
def initialize_logger(fileName: str):
    ...
    logFileName = (
        fileName + "-load-vep.log" if fileName else path.join(args.dir, "load-vep.log")
    )
...
def initialize_loader(fileName):
    ...
    try:
        ...
        loader.set_algorithm_invocation(
            "load_vep_result", print_args(args, False), commit=args.commit
        )
        ...
        loader.set_skip_existing(
            not args.updateExisting, args.gusConfigFile
...
def load(fileName):
    ...
    if not resume:
        LOGGER.info(
            "--resumeAfter flag specified; Finding skip until point %s",
            args.resumeAfter,
        )
        ...
        with open(fileName, "r") as fhandle, database.cursor() as cursor:
            ...
            with gzip.GzipFile(mode="r", fileobj=mappedFile) as gfh:
                ...
                for line in gfh:
                    ...
                    if (
                        loader.resume_load() != resume
                    ):  # then you are at the resume cutoff
                        resume = True
                        LOGGER.info("SKIPPED {:,} lines".format(lineCount))
                        continue
            ...
            LOGGER.info(
                "Consequences added during load: %s",
    ...
    if not args.fileName:
        if not args.chr:
            ...
            if not args.extension:
                die(
                    "Loading by chromosome, please provide file extension. Assume file names are like chr1.extension / TODO: pattern matching"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help="load 'commitAfter' rows as test"
    )
    parser.add_argument(
        "--resumeAfter",
    ...
    if args.fileName:
        ...
        chrList = (
            args.chr.split(",")
            if not args.chr.startswith("autosome")
