...
SELECT_SQL = "SELECT record_primary_key, metaseq_id FROM AnnotatedVDB.Variant WHERE chromosome = %s"
VARIANTS_PER_FILE = 10000000
ITERATION_SIZE = 500000
VCF_HEADER = qw('#CHRM POS ID REF ALT QUAL FILTER INFO', returnTuple=False)
INVALID_ALLELES = 'I|R|D|N'
        ...
        with database.named_cursor(cname, cursorFactory="RealDictCursor") as selectCursor, \
            ...
            for record in selectCursor:     
                ...
                if matches(INVALID_ALLELES, metaseqId):
                    warning("INVALID: " + primaryKey, file=lfh, flush=True)
                    continue
                
                if primaryKey in reference:
                    warning("DUPLICATE: " + primaryKey, file=lfh, flush=True)
                    continue
                else:
                    reference[primaryKey] = True
                    variants[primaryKey] = metaseqId
                
                if recordCount % ITERATION_SIZE == 0:
                    warning("Fetched " + xstr(recordCount) + " records from " + chromosome + ".", file=lfh, flush=True)
                    
                if recordCount % VARIANTS_PER_FILE == 0:
                    warning("Fetched " + xstr(recordCount) + " records from " + chromosome + ". Writing to file " + xstr(fileCount), file=lfh, flush=True)
                    print_file(variants, directory, chromosome, fileCount)
                    variants = {}
                    fileCount += 1
            
            # residuals
            warning("Fetched " + xstr(recordCount) + " records from " + chromosome + ". Writing residuals to file " + xstr(fileCount), file=lfh, flush=True)
            print_file(variants, directory, chromosome, fileCount)
