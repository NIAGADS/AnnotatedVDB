...
DEFAULT_COPY_FIELDS = qw(
    "chromosome record_primary_key position is_multi_allelic bin_index ref_snp_id metaseq_id display_attributes allele_frequencies adsp_most_severe_consequence adsp_ranked_consequences vep_output row_algorithm_id",
    returnTuple=False,
)
...
class VariantLoader(object):
    ...
    def initialize_variant_validator(self, gusConfigFile, useLegacyPK=False):
        ...
        self._variant_validator = VariantRecord(
            gusConfigFile, self._verbose, self._debug
        )
    ...
    def _verify_copy_fields(self, copyFields):
        ...
        if is_subset(copyFields, ALLOWABLE_COPY_FIELDS):
            if is_subset(REQUIRED_COPY_FIELDS, copyFields):
                ...
                raise ValueError(
                    "Copy fields must include the following required fields: "
                    + xstr(REQUIRED_COPY_FIELDS)
                )
        else:
            raise ValueError(
                "Copy fields include invalid columns from AnnotatedVDB.Variant: se variant_loader.py for list of allowable fields"
            )
    ...
    def initialize_pk_generator(self, genomeBuild, seqrepoProxyPath):
        ...
        self._pk_generator = VariantPKGenerator(
            genomeBuild, seqrepoProxyPath, debug=self._debug
        )
    ...
    def update_variants(self):
        ...
        if self._update_sql is None:
            raise ValueError(
                "must set update sql (VariantLoader.set_update_sql(sql) before attempting update"
            )
    ...
    def parse_variant(self, line):
        ...
        raise NotImplementedError(
            "Result parsing is not defined for the VariantLoader parent class, please use result-specific loader"
        )
