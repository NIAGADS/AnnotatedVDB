...
COPY_FIELDS = qw(
    "allele_frequencies adsp_most_severe_consequence adsp_ranked_consequences vep_output row_algorithm_id",
    returnTuple=False,
)
...
class VEPVariantLoader(VariantLoader):
    ...
    def initialize_vep_parser(self, rankingFile, rankConsequencesOnLoad, verbose):
        ...
        @param verbose
        """
        self.__vep_parser = VepJsonParser(
            rankingFile,
            rankConsequencesOnLoad=rankConsequencesOnLoad,
            verbose=verbose,
            debug=self._debug,
        )
    ...
    def __get_result_frequencies(self):
        ...
        """
        matchingVariantId = (
            self._current_variant.ref_snp_id if self.is_dbsnp() else None
        )
    ...
    def __clean_result(self):
        ...
        # FIXME: remove VCF entry; later need to add a check if it is already present
        # put in place for structural variants where it is already loaded.  In future, all
        # vcf-based loads will have vcf_entry field
    ...
    def __get_variant_primary_key(self, metaseqId):
        ...
        if result is None:
            raise KeyError(
                "No record for variant %s found in DB.  VEP Variant Loader does updates only.",
                metaseqId,
            )
        return result[0]["primary_key"]

    def __generate_sv_primary_key(self, chrom, start, end, svType):
        # FIXME: generate functions need to be moved to base class
        recordPK = self._pk_generator.generate_sv_primary_key(chrom, start, end, svType)
        return recordPK

    def __parse_structural_variant(self, vcfEntry: VcfEntryParser):
        # TODO - remove input from VEP annotation; already  stored in vcf_entry in DB
        # FIXME - this is the bareminimum for first load; revisit later
        ...
        if self._debug:
            self.logger.debug(f"Entry info: {vcfEntry.get('info')}")
        ...
        allele = cleanResult["allele_string"]
        msConseq = self.__vep_parser.get_most_severe_consequence(allele)
        ...
        updateValues = [
            ...
            xstr(self.__vep_parser.get_allele_consequences(allele), nullStr="{}"),
            xstr(cleanResult, nullStr="{}"),  # cleaned result JSON
            int(self._alg_invocation_id),
        ]

        if self.is_adsp():
            updateValues.append(True)  # is_adsp_variant

        if self._debug:
            self.logger.debug(f"Update Values: {updateValues}")
    ...
    def __parse_alt_alleles(self, vcfEntry):
        ...
        for alt in variant.alt_alleles:
            ...
            annotator = VariantAnnotator(
                variant.ref_allele, alt, variant.chromosome, variant.position
            )
            ...
            if self.has_attribute("vep_output", recordPK, returnVal=False):
                if self.skip_existing():  # otherwise will update existing record
                    ...
                    if self._log_skips:
                        self.logger.warning(
                            "Existing data found for: %s | %s; SKIPPING",
                ...
                if self._log_skips:
                    self.logger.warning(
                        "Existing data found for: %s | %s; UPDATING",
                        annotator.get_metaseq_id(),
                        variant,
                    )
    ...
    def parse_variant(self, line):
        ...
        if self._variant_validator is None:
            raise UnboundLocalError(
                "Validator has not been initialized, please execute the initialize_variant_validator method before parsing variants"
            )
        ...
        newConseqCount = (
            self.__vep_parser.get_consequence_parser().get_new_conseq_count()
        )
        ...
        try:
            ...
            entry = VcfEntryParser(
                self.__vep_parser.get("input"),
                identityOnly=self.is_adsp() and not self.is_structural_variant(),
            )
            ...
            if (
                ...
                self.logger.warning(
                    "New consequence added for %s - %s",
                    self._current_variant.id,
                    self.__vep_parser.get_consequence_parser().get_added_consequences(
                        mostRecent=True
                    ),
                ),

            if self.is_structural_variant():
                self.__parse_structural_variant(entry)
            else:
