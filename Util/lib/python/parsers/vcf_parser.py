...
class VcfEntryParser(object):
    ...
    def parse_entry(self, inputStr):
        ...
        try:
            entry = (
                ...
                else {field: values[index] for index, field in enumerate(fields)}
            )  # assume identityOnly
            ...
            raise IndexError(
                f"The number of fields in the VCF entry do not match number expected from provided VCF Header"
            )
        except Exception as err:
            raise ImportError(
                f"Unable to parse VCF entry: {inputStr}; ERROR: {str(err)}"
            )
    ...
    def get(self, key, raiseError=True) -> str:
    ...
    def infer_variant_end_location(self, alt):
        ...
        annotator = VariantAnnotator(
            self.get("ref"), alt, self.get("chrom"), int(self.get("pos"))
        )
