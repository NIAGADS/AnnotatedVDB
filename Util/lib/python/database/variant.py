...
PRIMARY_KEY_LOOKUP_SQL = (
    "record_primary_key = %s AND chromosome = 'chr' || split_part(%s, ':', 1)"
)
...
BULK_LOOKUP_FULL_SQL = (
    "SELECT * from get_variant_primary_keys_and_annotations(%s, %s, %s)"
)
# second param is "firstValueOnly flag"
BULK_LOOKUP_SQL = "SELECT * from map_variants(%s, %s, %s)"
# second param is "firstValueOnly flag", then checkAltVariants
SV_PRIMARY_KEY_LOOKUP = (
    "SELECT record_primary_key FROM AnnotatedVDB.Variant WHERE record_primary_key = %s"
)
...
class VariantRecord(object):
    ...
    def __init__(
        self, gusConfigFile=None, connectionString=None, verbose=False, debug=False
    ):
    ...
    def __validate_id_type(self, idType):
        ...
        if idType.upper() not in VARIANT_ID_TYPES:
            raise ValueError(
                idType + " not a valid variant ID type: " + VARIANT_ID_TYPES
            )
    ...
    def initialize_named_cursor(self, name: str, realDict=False, withhold=False):
        ...
        self.__cursors[name] = self.__database.named_cursor(
            name, cursorFactory=cursorFactory, withhold=withhold
        )
    ...
    def bulk_lookup(
        self, variants, firstHitOnly=True, fullAnnotation=True, checkAltVariants=True
    ):
    ...
    def has_json_attr(self, field, key, variantPK, returnVal=True):
        ...
        try:
            lookupIdType = (
                "LEGACY_PRIMARY_KEY"
                if idType == "PRIMARY_KEY" and self.legacy_pk()
                else idType
            )
            ...
            sql = (
                ...
                + "->>'"
                + key
                + "' FROM AnnotatedVDB.Variant WHERE "
                + LOOKUP_SQL[lookupIdType]
            )
            ...
            cursor = self.get_cursor(
                cursorKey, initializeIfMissing=True, realDict=False
            )
    ...
    def has_attr(self, field, variantPK, returnVal=True):
        ...
        try:
            ...
            sql = (
                "SELECT "
                + queryFields
                + " FROM AnnotatedVDB.Variant WHERE record_primary_key = %s"
            )
    ...
    def exists(self, variantId, returnMatch=False):
        ...
        try:
            ...
            cursor.execute(
                BULK_LOOKUP_SQL, (variantId, True, True)
            )  # firstHitOnly, checkAltAlleles
            result = cursor.fetchone()

            if result[0][variantId] is None:
                cursor.execute(SV_PRIMARY_KEY_LOOKUP, (variantId,))
                result = cursor.fetchone()
                if result is not None:
                    return result[0] if returnMatch else True
